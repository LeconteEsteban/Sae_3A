...
document.addEventListener("DOMContentLoaded", function () {
    ...
    toggleToRegister.addEventListener("click", function (event) {
        event.preventDefault(); // Empêche le lien de rafraîchir la page

        // Masque le formulaire de connexion et affiche l'inscription
    ...
    toggleToLogin.addEventListener("click", function (event) {
        ...
        // Masque l'inscription et affiche le login
  ...
  document.getElementById('loginFormElement').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Stocke les infos dans le local storage
            localStorage.setItem("user_id", data.user.user_id);
            localStorage.setItem("username", data.user.name);

            const previousPage = localStorage.getItem('previousPage') || "/";
            localStorage.removeItem('previousPage'); // Nettoyer après redirection
...
const loginForm = document.getElementById('loginFormElement');
console.log("🔎 loginFormElement trouvé ?", loginForm);
if (!loginForm) {
    console.error("⚠️ ERREUR : Impossible de basculer vers le login, loginFormElement introuvable !");
} else {
    switchToLogin();
}
